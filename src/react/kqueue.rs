...
use libc::{kqueue,kevent,EVFILT_READ,EV_ADD,EV_ONESHOT,timespec,close};
...
impl KqueueReactor {
    ...
    fn awake(&mut self, event: kevent) {
        ...
        self.ready.insert(event.ident as i32, event.data);
        /*
    ...
    fn poll_long(&mut self, fd: RawFd) {
    ...
    fn poll(&mut self, fd: RawFd, waker: Waker) -> Poll<isize> {
...
#[test] fn read_file() {
    ...
    while instant.elapsed() < Duration::from_secs(2) {
        ...
        let poll_result = KqueueReactor::shared().poll(fd, waker);
